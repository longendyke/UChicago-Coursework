...
TEST_F(test_person, Person) {
	Person Judah = Person("jgnewman" , "Judah" , "Newman" , "m" , 20 , "cs student"); 
	EXPECT_STREQ(Judah.get_firstname().c_str() , "Judah");
	EXPECT_STREQ(Judah.get_username().c_str() , "jgnewman");
	EXPECT_STREQ(Judah.get_lastname().c_str() , "Newman");
	EXPECT_STREQ(Judah.get_gender().c_str() , "m");
	EXPECT_STREQ(Judah.get_tagline().c_str() , "cs student");
	EXPECT_EQ(Judah.get_age() , 20);
	Person Invalid = Person("No" , "no" , "no" , "q" , 150 , "no"); 
	EXPECT_STREQ(Invalid.get_firstname().c_str() , "");
	EXPECT_STREQ(Invalid.get_username().c_str() , "");
	EXPECT_STREQ(Invalid.get_lastname().c_str() , "");
	EXPECT_STREQ(Invalid.get_gender().c_str() , "");
	EXPECT_STREQ(Invalid.get_tagline().c_str() , "");
	EXPECT_EQ(Invalid.get_age() , 0);
...
TEST_F(test_person, test_username) {
  ...
  EXPECT_STREQ(person.get_username().c_str(), "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijkl");
	EXPECT_TRUE(person.set_username("abcdefg09"));
...
TEST_F(test_person, test_gender) {
	...
	EXPECT_FALSE(person.set_gender("2"));
	EXPECT_FALSE(person.set_gender("q"));
