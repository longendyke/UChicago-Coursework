...
This output may not be too useful when you first start working on the project (and most of the tests will be failing), so you can also run only a single \emph{category} of tests like this:

\begin{example}
make grade CATEGORY=\emph{category}
\end{example}

Where \texttt{\emph{category}} is one of the following:

\begin{enumerate}
 \item Project 1a
 \begin{itemize}
  \item \verb+BASIC_CONNECTION+ 
 \end{itemize}

 \item Project 1b
 \begin{itemize}
\item \verb+CONNECTION_REGISTRATION+
\item \verb+PRIVMSG_NOTICE+
\item \verb+PING_PONG+
\item \verb+MOTD+
\item \verb+LUSERS+
\item \verb+WHOIS+
\item \verb+ERR_UNKNOWN+
\item \verb+ROBUST+
 \end{itemize}

 \item Project 1c
 \begin{itemize}
\item \verb+CHANNEL_JOIN+
\item \verb+CHANNEL_PRIVMSG_NOTICE+
\item \verb+CHANNEL_PART+
\item \verb+CHANNEL_TOPIC+
\item \verb+MODES+
\item \verb+AWAY+
\item \verb+NAMES+
\item \verb+LIST+
...
When a specific test fails in \texttt{make grade} (with or without \texttt{CATEGORY}), the test output will include the \emph{name} of the test that is failing. For example, if you see the following:
...
For each part of the project, 50\% of the project grade will be determined by the number of automated tests your solution passes successfully, as determined by running \texttt{make grade}. We will use the exact same tests provided in the upstream repository (i.e., there are no additional secret tests that only the graders have access to). So, if your solution passes 90\% of the tests before the deadline, you can be certain that you will get at least 45 out of 100 points (i.e., 90\% of 50).
...
\item Similarly, you can also test your implementation with an existing IRC client. We recommend using \texttt{irssi} (\url{http://irssi.org/}), which is installed on the CS Linux machines.
...
Take into account that you should expect a \texttt{<user>} parameter but will ignore its content; the password expected by the \texttt{OPER} command is the one specified in the \texttt{-o} command-line parameter to the \texttt{chirc} executable.
...
\begin{itemize}
\item When only a single parameter (a channel name) is used, the only error condition you must support is the \texttt{ERR\_NOSUCHCHANNEL} reply (although this is not included in the specification for \texttt{MODE}). If the command is successful, return a \texttt{RPL\_CHANNELMODEIS} reply (in this reply, the  \texttt{<mode>} parameter must be a plus sign followed by the channel modes; you must omit the \texttt{<mode params>} parameter).
\item When two parameters (a channel name and a mode string) are used, you must support the following error replies: \texttt{ERR\_NOSUCHCHANNEL}, \texttt{ERR\_CHANOPRIVSNEEDED}, and \texttt{ERR\_UNKNOWNMODE}. If the command is successful, the message is relayed back to the user and to all the users in the channel.
\end{itemize}
